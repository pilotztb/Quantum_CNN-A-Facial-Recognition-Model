...
# --- 用于更清晰图像的参数 ---
high_dpi = 300  # 您可以尝试像 150, 300, 或 600 这样的值
image_scale = 1.0 # Qiskit的draw方法的scale参数，默认为0.7。尝试大于1的值使元件更大。
# 如果需要，可以考虑一个稍大的图像尺寸，例如 figsize=(10, 4) 用于较宽的电路
custom_figsize = (max(8, num_qnn_qubits * 0.5), max(4, num_qnn_qubits * 0.3)) # 根据量子比特数动态调整
...
if num_qnn_qubits <= 10: # 保留绘制条件，或者如果您也想绘制更大的电路，则进行调整
    try:
        # 生成 PNG 格式
        fig_fm_png, ax_fm_png = plt.subplots(figsize=custom_figsize) # 应用figsize
        feature_map.decompose().draw("mpl", ax=ax_fm_png, fold=-1, scale=image_scale) # 添加了scale参数
        png_path_fm = os.path.join(output_image_folder, f"03_QNN_{feature_map.name}({num_qnn_qubits}qubits)_dpi{high_dpi}.png")
        fig_fm_png.savefig(png_path_fm, dpi=high_dpi, bbox_inches='tight') # 添加了dpi和bbox_inches参数
        plt.close(fig_fm_png)
        logging.info(f"QNN 特征映射 ({feature_map.name}) PNG图像已保存到: {png_path_fm}")

        # 生成 SVG 格式 (推荐，以保证清晰度)
        fig_fm_svg, ax_fm_svg = plt.subplots(figsize=custom_figsize)
        ...
        fig_fm_svg.savefig(svg_path_fm, format='svg', bbox_inches='tight') # 保存为SVG格式
...
if num_qnn_qubits <= 10: # 保留绘制条件或进行调整
    try:
        # 生成 PNG 格式
        fig_ans_png, ax_ans_png = plt.subplots(figsize=custom_figsize) # 应用figsize
        ansatz.decompose().draw("mpl", ax=ax_ans_png, fold=-1, scale=image_scale) # 添加了scale参数
        png_path_ans = os.path.join(output_image_folder, f"04_QNN_{ansatz.name}({num_qnn_qubits}qubits)_dpi{high_dpi}.png")
        fig_ans_png.savefig(png_path_ans, dpi=high_dpi, bbox_inches='tight') # 添加了dpi和bbox_inches参数
        plt.close(fig_ans_png)
        logging.info(f"QNN Ansatz ({ansatz.name}) PNG图像已保存到: {png_path_ans}")

        # 生成 SVG 格式 (推荐，以保证清晰度)
        fig_ans_svg, ax_ans_svg = plt.subplots(figsize=custom_figsize)
        ...
        fig_ans_svg.savefig(svg_path_ans, format='svg', bbox_inches='tight') # 保存为SVG格式
...
qnn_classifier = NeuralNetworkClassifier(
    ...
    optimizer=COBYLA(maxiter=150), # 修改：增加 COBYLA 的迭代次数
    callback=simple_text_callback,
    loss='cross_entropy',  
    one_hot=True,          
    initial_point=initial_point_qnn,
)
logging.info("NeuralNetworkClassifier (多分类，使用内置 'cross_entropy' 损失，one_hot=True, COBYLA maxiter=150) 已定义。")
...
if objective_func_vals:
    try:
        ...
        plt.title('QNN (多分类，内置CE, FM_reps3, Ans_reps5, COBYLA150) 训练目标函数值') # 更新标题
        plt.xlabel('迭代次数'); plt.ylabel('目标函数值 (交叉熵)'); plt.grid(True)
        # 更新文件名以反映更改
        plt.savefig(os.path.join(output_image_folder, "06_QNN_多分类_FM3_Ans5_COBYLA150_目标函数值.png"))
        plt.close()
